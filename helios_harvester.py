# helios_harvester.py (v6.0 - Grand Design Edition)
import requests
import json
...
def generate_narrative_vector(data, previous_data):
    try:
        ...
        for name, archetype in HISTORICAL_ARCHETYPES.items():
            ...
            funding_dist = (funding*1000 - archetype["funding"]*1000)**2
            distance = math.sqrt(rsi_dist + funding_dist)
            if distance < 10:
                return {"primary": f"ECHO DETECTED: {archetype['insight']}", "market": "High-conviction pattern match. Standard market analysis suspended.", "network": "On-chain fundamentals are secondary to the recognized historical echo."}
        ...
        if funding > 0.0002: market_analysis += "Speculative longs are paying a premium, indicating bullish conviction."
        ...
        return {"primary": primary_insight, "market": market_analysis, "network": network_analysis}
...
def fetch_market_vector():
    try:
        ...
        result = data.get("result", {}).get("XXBTZUSD", {}); return {"price_usd": float(result.get('c', [0])[0])} if result else None
...
def fetch_trend_vector(previous_price):
    try:
        ...
        closing_prices = [float(item[4]) for item in result]; rsi_14d = calculate_rsi(closing_prices)
...
def fetch_speculative_vector():
    try:
        ...
        oi_response.raise_for_status(); open_interest = float(oi_response.json().get("result", {}).get("list", [{}])[0].get("openInterest", 0))
...
def main():
    ...
    if not all([vectors["market_vector"], vectors["mining_vector"], vectors["monetary_vector"]]): print("[ERROR] Failed to forge CORE vectors. Aborting."); return
