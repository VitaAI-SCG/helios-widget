# helios_harvester.py (v5.1 - ALPHA Analyst Edition)
import requests
import json
...
# --- CONFIGURATION & DATA SOURCES (Unchanged) ---
...
# --- CORE ENGINES (Unchanged from v5.0) ---
...
# --- LEGENDARY "ALPHA ANALYST" AI ---
def generate_narrative_vector(data, previous_data):
    """Generates a multi-faceted analysis with the ALPHA Analyst personality."""
    try:
        ...
        # --- Primary Insight (The Main Briefing) ---
        ...
        if rsi > 70: primary_insight = f"Market sentiment is overheated (RSI: {rsi:.0f}). A correction is probable."
            ...
            direction = "expanding" if price_change > 0 else "contracting"
            primary_insight = f"Financial energy is {direction}. The market has chosen its short-term direction."

        # --- Market Analysis ---
        market_analysis = f"Momentum is neutral (RSI: {rsi:.1f}). "
        if funding_rate > 0.0002: market_analysis += "Speculative longs are paying a premium, indicating bullish conviction."
        elif funding_rate < -0.0001: market_analysis += "Speculative shorts are dominant, indicating bearish conviction."
        else: market_analysis += "The derivatives market is balanced."

        # --- Network Analysis ---
        network_analysis = f"The {diff_change:.2f}% difficulty adjustment indicates the security baseline is "
        if diff_change > 1: network_analysis += "currently hardening."
        elif diff_change < -1: network_analysis += "currently optimizing."
        else: network_analysis += "holding steady."
...
# --- DATA FETCHING (Unchanged from v5.0) ---
def fetch_market_vector():
    try:
        response = requests.get(f"{KRAKEN_API_URL}/Ticker?pair=XBTUSD", headers={"User-Agent": "Helios/5.1"})
...
def fetch_trend_vector(previous_price):
    try:
        response = requests.get(f"{KRAKEN_API_URL}/OHLC?pair=XBTUSD&interval=1440", headers={"User-Agent": "Helios/5.1"})
        response.raise_for_status(); data = response.json()
        result = data.get("result", {}).get("XXBTZUSD", [])
...
def fetch_mining_vector():
    try:
        req = urllib.request.Request(f"{MEMPOOL_API_URL}/v1/difficulty-adjustment", headers={"User-Agent": "Helios/5.1"})
        ...
        try:
            req = urllib.request.Request(f"{BLOCKSTREAM_API_URL}/difficulty-adjustment", headers={"User-Agent": "Helios/5.1"})
            with urllib.request.urlopen(req) as r: data = json.loads(r.read().decode('utf-8'))
            return {"difficulty_change_percent": data.get('difficultyChange'), "next_retarget_blocks": data.get('remainingBlocks')}
        except Exception as e2: print(f"[ERROR] Mining Vector (fallback failed): {e2}"); return None
def calculate_monetary_vector():
    try:
        response = requests.get(f"{MEMPOOL_API_URL}/blocks/tip/height", headers={"User-Agent": "Helios/5.1"})
        response.raise_for_status(); current_block = int(response.text)
    except Exception as e:
        print(f"[WARN] Primary monetary source failed: {e}. Trying fallback.")
        try:
            response = requests.get(f"{BLOCKSTREAM_API_URL}/blocks/tip/height", headers={"User-Agent": "Helios/5.1"})
            response.raise_for_status(); current_block = int(response.text)
...
def fetch_speculative_vector():
    try:
        oi_response = requests.get(f"{BYBIT_API_URL}/open-interest?category=linear&symbol=BTCUSDT", headers={"User-Agent": "Helios/5.1"})
        oi_response.raise_for_status()
        open_interest = float(oi_response.json().get("result", {}).get("list", [{}])[0].get("openInterest", 0))
        funding_response = requests.get(f"{BINANCE_API_URL}/premiumIndex?symbol=BTCUSDT", headers={"User-Agent": "Helios/5.1"})
...
def main():
    print(f"[{datetime.now().isoformat()}] Forging Helios Data Core with ALPHA Analyst Engine...")
    previous_data = None;
