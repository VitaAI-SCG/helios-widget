# helios_harvester.py (v2.1 - Anti-Fragile Engine)
import requests
import json
...
def generate_narrative_vector(current_data, previous_data):
    ...
    try:
        ...
        speculative_vector = current_data.get("speculative_vector")
        current_funding_rate = speculative_vector.get("funding_rate") if speculative_vector else None
        ...
        if previous_data:
            ...
            if abs(price_change_percent) > 0.1: # Significant price movement
                ...
                if current_funding_rate and current_funding_rate > 0.0002: # High funding
                    insight = f"Financial energy is {direction}, fueled by aggressive speculative sentiment."
                elif current_funding_rate and current_funding_rate < 0: # Negative funding
...
def fetch_market_vector():
    ...
    try:
        response = requests.get(KRAKEN_API_URL, headers={"User-Agent": "Helios/2.1"})
...
def fetch_mining_vector():
    ...
    try:
        req = urllib.request.Request(f"{MEMPOOL_API_URL}/v1/difficulty-adjustment", headers={"User-Agent": "Helios/2.1"})
...
def calculate_monetary_vector():
    ...
    try:
        response = requests.get(f"{MEMPOOL_API_URL}/blocks/tip/height", headers={"User-Agent": "Helios/2.1"})
...
def fetch_speculative_vector():
    """Fetches speculative data. If it fails, it returns a default object, making the system resilient."""
    try:
        oi_response = requests.get(f"{BINANCE_FUTURES_API_URL}/openInterest?symbol=BTCUSDT", headers={"User-Agent": "Helios/2.1"})
        oi_response.raise_for_status(); open_interest = float(oi_response.json().get("openInterest", 0))
        funding_response = requests.get(f"{BINANCE_FUTURES_API_URL}/premiumIndex?symbol=BTCUSDT", headers={"User-Agent": "Helios/2.1"})
